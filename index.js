...
// Simple server for uptime
// Improved web server for better uptime monitoring
app.get('/', (req, res) => {
  res.send({
    status: 'active',
...
const keepAliveInterval = 2 * 60 * 1000; // Every 2 minutes
let lastPingTime = Date.now();

const keepAlive = async () => {
  try {
    const timestamp = new Date().toISOString();
    console.log('Bot keep-alive ping:', timestamp);
    
    // Multiple ping attempts for reliability
    await Promise.all([
      fetch('https://57046f84-8fe4-4e2d-86b0-28a0156e437b-00-28bouksc6dxxk.pike.replit.dev/').catch(() => null),
      fetch('https://57046f84-8fe4-4e2d-86b0-28a0156e437b-00-28bouksc6dxxk.pike.replit.dev/ping').catch(() => null)
    ]);

    lastPingTime = Date.now();
  } catch (err) {
    console.error('Keep-alive error:', err);
    // Attempt immediate retry on failure
